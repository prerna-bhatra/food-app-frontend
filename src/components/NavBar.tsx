    ...
    <nav className={` md:px-16 border border-gray-300 mb-100 md:mt-0 pb-4 ${pathsThatInludesOnlyProfile.includes(location.pathname) ? 'main-nav-bar shadow' : 'nav-bar  pb-[8rem]'}`}>
      <div className="container mx-auto flex justify-between items-center mt-6">
        ...
        <div className="md:block space-x-4">
          {token ? (
            <div className="flex items-center space-x-4">
              <button onClick={() => handleNavButtonClick("my-restaurants")} className={`flex items-center space-x-1 focus:outline-none  hidden md:block ${pathsThatInludesOnlyProfile.includes(location.pathname) ? 'text-black' : 'text-white'}`}>
                My restaurants
              </button>
              <button onClick={() => handleNavButtonClick("partner-with-us")} className={`flex items-center space-x-1 focus:outline-none  hidden md:block ${pathsThatInludesOnlyProfile.includes(location.pathname) ? 'text-black' : 'text-white'}`}>
                Register restaurant
              </button>
              <button className={`flex items-center space-x-1 focus:outline-none  hidden md:block ${pathsThatInludesOnlyProfile.includes(location.pathname) ? "text-black" : "text-white"}`}>
                <span>Offers</span>
              </button>
              <button className={`flex items-center space-x-1 focus:outline-none  hidden md:block ${pathsThatInludesOnlyProfile.includes(location.pathname) ? "text-black" : "text-white"}`}>
                <span>Cart</span>
        ...
        !pathsThatInludesOnlyProfile.includes(location.pathname) ? (
          <>
            ...
            {(menus && menus.length) || (restaurants && restaurants.length) ? (
              <div className='max-h-80 w-[874px] mx-20 rounded-[24px] overflow-hidden z-50 relative bg-white p-4 mt-4'>
